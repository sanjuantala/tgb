...
class tianlock_wp_recentposts extends WP_Widget {
     function __construct() {
      $widget_ops = array('description' => esc_html__('Your site\'s most recent Posts.', 'tianlock-wp'));
        parent::__construct(false, $name = ''. esc_html__('Custom: Recent Posts', 'tianlock-wp') .'',$widget_ops); 
     }
...
<?php $tianlock_wp_latest = new WP_Query(array('post_type' => 'post', 'ignore_sticky_posts' => 1, 'posts_per_page' => esc_attr($number) )); // number to display more / less ?>
<?php while ( $tianlock_wp_latest->have_posts() ) : $tianlock_wp_latest->the_post(); ?>
...
<?php  } } 
add_action('widgets_init', create_function('', 'return register_widget("tianlock_wp_recentposts");')); // register widget
?>