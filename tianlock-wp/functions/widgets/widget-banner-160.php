...
class tianlock_wp_160px extends WP_Widget {
     function __construct() {
	    $widget_ops = array('description' => esc_html__('Advertisement - Paste your HTML or JavaScript code.', 'tianlock-wp'));
        parent::__construct(false, $name = ''. esc_html__('Custom: Advertisement 160px', 'tianlock-wp') .'',$widget_ops); 
    }
...
<?php if ( $title_tw ) echo wp_kses_post($before_title) . esc_attr( $title_tw ) . wp_kses_post($after_title); ?>
  ...
  <div class="img-160"><?php echo stripslashes_deep($bcode); // esc_textarea() if is added will be shown as a text ?></div>
...
<?php  } }
add_action('widgets_init', create_function('', 'return register_widget("tianlock_wp_160px");')); // register widget
?>