...
class tianlock_wp_topposts extends WP_Widget {
     function __construct() {
      $widget_ops = array('description' => esc_html__('Popular Posts - Top Posts by comments.', 'tianlock-wp'));
      parent::__construct(false, $name = ''. esc_html__('Custom: Top Posts', 'tianlock-wp') .'',$widget_ops); 
    }
...
<?php $tianlock_wp_antop = new WP_Query(array('orderby' => 'comment_count', 'ignore_sticky_posts' => 1, 'posts_per_page' => esc_attr($number) )); // number to display more / less ?>
<?php while ($tianlock_wp_antop->have_posts()) : $tianlock_wp_antop->the_post(); ?> 
...
<?php  } } 
add_action('widgets_init', create_function('', 'return register_widget("tianlock_wp_topposts");')); // register widget
?>