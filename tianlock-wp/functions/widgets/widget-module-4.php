...
class tianlock_wp_module4 extends WP_Widget {
     function __construct() {
	    $widget_ops = array('description' => esc_html__('Module 4: Top Liked Posts', 'tianlock-wp'));
        parent::__construct(false, $name = ''. esc_html__('== Module 4 == Top Liked Posts', 'tianlock-wp') .'',$widget_ops); 
    }
...
<?php if ( $title ) echo wp_kses_post($before_title) . esc_attr($title) . wp_kses_post($after_title); ?>
    ...
    <ul class="articles-modules">
        <?php // The Query
        $tianlock_wp_toplikes = new WP_Query(array('meta_key' => '_thumbs_rating_up', 'ignore_sticky_posts' => 1, 'orderby' => 'meta_value_num', 'order' => 'DESC', 'posts_per_page' => esc_attr(5) )); // number to display more / less ?>
...
<?php  } }
add_action('widgets_init', create_function('', 'return register_widget("tianlock_wp_module4");')); // register widget
?>