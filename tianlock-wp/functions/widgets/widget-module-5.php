...
class tianlock_wp_module5 extends WP_Widget {
     function __construct() {
	    $widget_ops = array('description' => esc_html__('Module 5: Random Posts', 'tianlock-wp'));
        parent::__construct(false, $name = ''. esc_html__('== Module 5 == Random Posts', 'tianlock-wp') .'',$widget_ops); 
    }
...
<?php if ( $title ) echo wp_kses_post($before_title) . esc_attr($title) . wp_kses_post($after_title); ?>
    ...
    <ul class="articles-modules">
          <?php $tianlock_wpwidget_randomposts = new WP_Query(array('orderby' => 'rand', 'ignore_sticky_posts' => 1, 'posts_per_page' => esc_attr(5) )); ?>
          <?php while ($tianlock_wpwidget_randomposts->have_posts()) : $tianlock_wpwidget_randomposts->the_post(); ?> 
...
<?php  } }
add_action('widgets_init', create_function('', 'return register_widget("tianlock_wp_module5");')); // register widget
?>