...
class tianlock_wp_module3 extends WP_Widget {
     function __construct() {
	    $widget_ops = array('description' => esc_html__('Module 3: Top Posts By comments', 'tianlock-wp'));
        parent::__construct(false, $name = ''. esc_html__('== Module 3 == Top Posts By comments', 'tianlock-wp') .'',$widget_ops); 
    }
...
<?php if ( $title ) echo wp_kses_post($before_title) . esc_attr($title) . wp_kses_post($after_title); ?>
    ...
    <nav class="menu--adsila">
        <div class="menu__item">
          <span class="menu__item-name"><?php echo esc_attr( $redtitle ); ?></span>
          <span class="menu__item-label"><?php echo esc_attr( $blacktitle ); ?></span>
        </div>
    </nav>

    <!-- Articles Module -->
    <ul class="articles-modules">
        <?php $tianlock_wp_antop = new WP_Query(array('orderby' => 'comment_count', 'ignore_sticky_posts' => 1, 'posts_per_page' => esc_attr(5) )); // number to display more / less ?>
        <?php $num=1; while ($tianlock_wp_antop->have_posts()) : $tianlock_wp_antop->the_post(); ?> 
...
<?php
  ...
  function form( $instance ) {
    $defaults  = array( 'title' => '', 'redtitle' => '', 'blacktitle' => '');
...
<?php  } }
add_action('widgets_init', create_function('', 'return register_widget("tianlock_wp_module3");')); // register widget
?>